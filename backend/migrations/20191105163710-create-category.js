...
module.exports = {
  up: (queryInterface, Sequelize) => {
    ...
    return queryInterface.sequelize
      .transaction((transaction) => createCategories(transaction)
        .then(() => addCategoryIdToPost(transaction)));
  },
  down: (queryInterface) => {
    const removeCategorytable = (transaction) => queryInterface.dropTable('Categories', {transaction});

    const removeCategoryIdFromPost = (transaction) => queryInterface.describeTable('Posts')
      .then((tableDefinition) => {
        if (tableDefinition.categoryId) {
    ...
    return queryInterface.sequelize
      .transaction((transaction) => removeCategoryIdFromPost(transaction)
